...
pub fn init_world(harness: &mut Harness) {
    ...
    // fluids.
    let history = std::fs::read_to_string(std::env::args().collect::<Vec<String>>().get(1).unwrap().as_str()); 
...
impl NonPressureForce for CustomForceField {
    fn solve(
        ...
        for (pos, acc) in fluid.positions.iter().zip(fluid.accelerations.iter_mut()) {
            let points = fibonacci_sphere::fib_sphere::points_on_sphere_fib(state.state.npoints, 1.0);
